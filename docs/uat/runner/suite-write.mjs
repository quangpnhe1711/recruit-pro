...
export async function runWrite() {
  ...
  const cand = await login("haidang"); // spare candidate (not the primary nhatquang)
  ...
  await testCase("UAT-FILE-W01", "Candidate uploads a resume (real PDF)", "P1", async () => {
    const pdfUrl = new URL("../../../../cv/PhungNhatQuang-5_3_26.pdf", import.meta.url);
    const bytes = readFileSync(pdfUrl);
    const fd = new FormData();
    fd.append("resume", new Blob([bytes], { type: "application/pdf" }), "PhungNhatQuang-5_3_26.pdf");
    const r = await api("POST", "/candidate/profile/resume", { token: candTok, body: fd });
    expect([200, 201].includes(r.status), `resume upload HTTP ${r.status} ${r.json?.errorCode || r.text?.slice(0,150)}`);
    return { actualApi: `POST /candidate/profile/resume (multipart) → ${r.status}` };
  });
